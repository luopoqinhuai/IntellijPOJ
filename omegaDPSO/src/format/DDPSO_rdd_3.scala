...
class DDPSO_rdd_3 (graph:Graph[gen,Int],vertexNum:Int)extends Serializable {
  ...
  def init(sc:SparkContext,nPopulationNum:Int,partitionNum:Int,mlambda:Double):RDD[(Long,gen)]= {
    ...
    population.count()
    val fitnessupdate=upDateFitness(population)
  ...
  def testFitness(mPoputation:RDD[(Long,gen)])={
    var outJoinVerticesTime=0.0
    var outAggregrateTime=0.0
    var outFitnessTimepart_1=0.0
    var outThirdTime=0.0
    val sc=mPoputation.sparkContext
    var  startTime=System.currentTimeMillis()
    val tmpGraph:Graph[gen,Int]=graph.joinVertices(mPoputation){(vid,vdata ,U)=>U}.cache()
    tmpGraph.edges.count()
    tmpGraph.vertices.count()
    var  endTime=System.currentTimeMillis()
    outJoinVerticesTime=endTime-startTime
    startTime=System.currentTimeMillis()
    //(VID,gen)
    // classID    ,Nbest         ,SameNum     DiffNum
    ...
    outAggregrateTime=endTime-startTime
    startTime=System.currentTimeMillis()

    //将一个RDD拆分成 个体数目个RDD
    val mfitness:Array[Double]=(0 to populationNum-1).map{i=>
      val pClass=tp.map(x=>(x._2._1(i),(x._2._3(i),x._2._4(i)))).groupByKey.cache()
      pClass.count()
    ...
    outFitnessTimepart_1=endTime-startTime
    ...
    outThirdTime=endTime-startTime

    mPoputation.unpersist()
    tmpGraph.unpersistVertices()
    tmpGraph.edges.unpersist()
    tp.unpersist()
    (outJoinVerticesTime,outAggregrateTime,outFitnessTimepart_1,outThirdTime)
  }



  def upDateFitness(mPoputation:RDD[(Long,gen)]):RDD[(Long,gen)]={
    require(populationNum>0)
    val sc=mPoputation.sparkContext
    val tmpGraph:Graph[gen,Int]=graph.joinVertices(mPoputation){(vid,vdata ,U)=>U}//可更改
    //(VID,gen)
                                     // classID    ,Nbest         ,SameNum     DiffNum
    val tp=tmpGraph.aggregateMessages[(Array[Int],Array[List[Int]],Array[Int],Array[Int])](
        triplet => {
